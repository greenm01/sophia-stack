...
grep -q '^sophia_qemu_xmonad_pointer schema=5 status=sent source=qmp device=virtio-mouse action=focus_click anchor=left_content inset=32 clicks=1 commands=4$' "$evidence"
grep -q '^sophia_qemu_xmonad_pointer schema=2 status=sent source=qmp device=virtio-mouse action=focus_drag anchor=left_content inset=32 drag=96x24 commands=5$' "$evidence"
