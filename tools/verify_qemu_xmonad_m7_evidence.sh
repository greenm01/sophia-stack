...
grep -Eq '^sophia_qemu_xmonad_pointer schema=5 status=sent source=qmp device=virtio-mouse action=focus_click anchor=left_content inset=32 clicks=1 commands=4$|^sophia_qemu_xmonad_pointer schema=6 status=sent source=qmp device=virtio-mouse action=focus_click anchor=left_content x_inset=32 y_steps=8x16 clicks=1 commands=13$' "$evidence"
grep -Eq '^sophia_qemu_xmonad_pointer schema=2 status=sent source=qmp device=virtio-mouse action=focus_drag anchor=left_content inset=32 drag=96x24 commands=5$|^sophia_qemu_xmonad_pointer schema=3 status=sent source=qmp device=virtio-mouse action=focus_drag anchor=left_content x_inset=32 y_steps=8x16 drag=96x24 commands=14$' "$evidence"
