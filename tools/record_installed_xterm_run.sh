...
SOPHIA_ATTEMPT_KIND=xterm
SOPHIA_ATTEMPT_PROFILE=xmonad
SOPHIA_ATTEMPT_RESULT_RECORD=sophia_installed_xterm
SOPHIA_ATTEMPT_RUN_ROOT="${SOPHIA_XTERM_RUN_ROOT:-$STATE_HOME/sophia/promotion/xterm-runs}"
...
SOPHIA_ATTEMPT_VERIFY_SESSION="${SOPHIA_VERIFY_XTERM_BIN:-$RELEASE_DIR/bin/sophia-verify-xterm-run}"
...
if [[ ! -x "$SOPHIA_ATTEMPT_VERIFY_SESSION" ]]; then
    SOPHIA_ATTEMPT_VERIFY_SESSION="$RELEASE_DIR/tools/verify_installed_xterm_session.sh"
fi
