...
| xmobar status bar | `cargo run --offline -q -p sophia-cli -- x-authority-xmobar-smoke` plus `tools/start_sophia_xmonad_tty3.sh` with `SOPHIA_ENABLE_XMOBAR=true` | unmodified xmobar creates an override-redirect top bar that remains client-positioned, Engine-composed, and outside the blind WM layout stream | `session` | `automated proof passed; physical proof pending`: unmodified xmobar 0.51.1 completed 163 requests across 28 opcodes with two committed CPU-buffer transactions, 8,967 nonzero bytes, and `first_error=none`. Exact MIT-SHM pixmap upload/readback and copy-to-window semantics drive the pixels. The frontend emits only a protocol-neutral presentation role, and the launcher supervises an installed or isolated local-checkout build. `_NET_WM_STRUT_PARTIAL` work-area reservation remains an explicit follow-up. |
