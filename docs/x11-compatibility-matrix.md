...
| Kitty interactive session | `cargo run --offline -q -p sophia-cli -- x-authority-kitty-input-smoke` plus `tools/start_sophia_kitty_tty3.sh` | real Kitty GLX/DRI3 startup, routed keyboard and pointer input, Present feedback, native two-output scanout, normal exit, and TTY/display-manager restoration | `session` | `proven` with installed Kitty 0.48.0 and the guarded 2026-07-24 TTY3 run: the automated gate routes six core key events, receives exact `ll`, and observes three post-input Presents; the physical run reached readiness in 798 ms, routed keyboard and two button transitions, bounded cursor motion-to-submit at 13 ms, exited Kitty with status zero, retained clean protocol health, and restored the originating TTY without emergency recovery. Traditional extension ranges are non-core and mutually disjoint. This promotes Kitty-only operation, not the physical xmonad desktop. |
