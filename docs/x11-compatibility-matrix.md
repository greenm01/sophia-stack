...
| Firefox native-X trace | `cargo run --offline -q -p sophia-cli -- x-authority-firefox-smoke` | bounded native-X startup trace against an owner-local Sophia display, forced X11 backend, isolated profile, process-group cleanup, nonzero pixels, and semantic expected/unexpected protocol-error reporting | `client` | `proven` for standalone startup: the retained host run completed 396 requests across 45 opcodes, two runtime surfaces, 5,971,968 CPU-buffer bytes, nonzero pixels, and `first_error=none`. The offline local-page interaction sequence and M8 two-output witness remain promotion gates for the daily-driver session. |
