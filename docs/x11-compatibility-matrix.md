...
| Firefox native-X trace and daily-driver session | `cargo run --offline -q -p sophia-cli -- x-authority-firefox-smoke`, `tools/qemu_xmonad_m8_mix_acceptance.sh`, and `tools/qemu_xmonad_m8_soak_acceptance.sh` | bounded native-X startup plus an offline eight-stage keyboard, pointer, real document navigation/scroll, resize, refocus, attached dialog, `CLIPBOARD`, `PRIMARY`, close, and cleanup proof in the two-output xmonad session | `session` | `proven`: the standalone trace completed 396 requests across 45 opcodes with no unexpected error; three consecutive mixed gates passed; the retained 1,891,936 ms soak completed 22 Firefox restarts and 66 total managed closes with zero unexpected protocol errors and clean teardown. Event sequences are socket-write ordered, and multi-toplevel close selects exact, ancestor, or unique `WM_DELETE_WINDOW` targets before bounded termination. The strengthened navigation, popup-lifecycle, and three-surface resize contract still requires a fresh physical Milestone 10 run. |
...
| Classic direct GLX bootstrap | `cargo run --offline -q -p sophia-cli --features atomic-scanout-live -- x-authority-glxgears-smoke` | depth-capable double-buffered visual selection, legacy direct-context creation, DRI3 import, and Present submission | `engine` | `proven` by the bounded real-Mesa preflight with every required wire stage and no X protocol error. Direct Mesa does not send MakeCurrent over the wire for this workload; Sophia nevertheless implements validated legacy MakeCurrent for clients that do. Physical animation and KMS cadence remain pending. |
| GLX offscreen drawables | `cargo test --offline -q -p sophia-x-authority --test x11_wire glx_pbuffer` | GLX 1.3 pbuffer create and destroy, core `GetGeometry` on a GLX drawable, intrinsic drawable attributes, `MakeContextCurrent` | `wire` | `proven` at the wire and dispatch boundary only. A physical GL client has not yet exercised it: the offline mesa-demos probe intended to prove the Mesa/DRI3 half does not run, so no real-client evidence exists. GLX 1.3 minors 13, 15, 22 and 23 (GLX pixmaps), 25 `QueryContext` and 30 `ChangeDrawableAttributes` stay unimplemented and answer `BadRequest`; `GLX_PIXMAP_BIT` is withdrawn so the advertised drawable types are exactly the implemented ones. Proven against real Mesa by `x-authority-glx-pbuffer-smoke`, which drives `pbdemo` through a GLX pbuffer and exits zero. The GLX 1.3 request surface is complete apart from GLX pixmaps (13, 15, 22, 23), which are withdrawn rather than promised; the indirect-GLX requests (1, 2, 8, 9, 10, 11, 12, 16, 17) are excluded by architecture, since Sophia renders client-side and imports through DRI3. GLX 1.4 adds only multisample attributes over 1.3, and those are advertised. Two identified causes remain outside GLX: DRI3 minor 8 `BuffersFromPixmap` is unimplemented while DRI3 1.2 is advertised, and ANGLE's ES-profile fallback requires `GLX_EXT_create_context_es_profile`, which is not advertised. |
| Kitty interactive session | `cargo run --offline -q -p sophia-cli -- x-authority-kitty-input-smoke` plus `tools/start_sophia_kitty_tty3.sh` | real Kitty GLX/DRI3 startup, routed US keyboard and pointer input, Present feedback, native two-output scanout, normal exit, and TTY/display-manager restoration | `session` | `proven` with installed Kitty 0.48.0 and the guarded 2026-07-24 TTY3 run: the automated gate routes ten core key events, receives exact `:ll` including Shift+semicolon XKB state, and observes three post-input Presents; the physical run reached readiness in 798 ms, routed keyboard and two button transitions, bounded cursor motion-to-submit at 13 ms, exited Kitty with status zero, retained clean protocol health, and restored the originating TTY without emergency recovery. Traditional extension ranges are non-core and mutually disjoint. This promotes Kitty-only operation, not the physical xmonad desktop. |
| xmobar status bar | `cargo run --offline -q -p sophia-cli -- x-authority-xmobar-smoke` plus `tools/start_sophia_xmonad_tty3.sh` with `SOPHIA_ENABLE_XMOBAR=true` | unmodified xmobar creates an override-redirect top bar that remains client-positioned, Engine-composed, and outside the blind WM layout stream | `session` | `automated proof passed; corrected physical proof pending`: unmodified xmobar 0.51.1 completed 215 requests across 28 opcodes with six committed CPU-buffer transactions, 25,929 nonzero bytes, and `first_error=none`; the gate now requires three sustained redraws and nonzero pixels in the newest transaction. Two physical runs exposed generic mixed-renderer boundaries in sequence: flattened CPU pixels were occluded by Kitty, then the corrected bar-sized CPU overlay was rejected by a legacy full-output texture guard. Mixed presentation now interleaves both buffer classes by Engine order and persistently reuses layer-sized CPU textures. The frontend remains application-agnostic, and neither xmobar nor xmonad is modified. `_NET_WM_STRUT_PARTIAL` work-area reservation remains an explicit follow-up. |
