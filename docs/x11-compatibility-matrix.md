...
| GTK software dialog | Engine smoke plus `tools/live_session_milestone5_gtk_hardware_proof.sh` | GTK negotiates XKB/XGE/XI2, reads bounded RandR output properties, subscribes to XFixes selection changes, commits nonzero `MIT-SHM` pixels, accepts exact physical text and an OK-button click, redraws after resize, and exits normally | `engine` | Engine coverage is `proven` with `first_error=none` and 288,920 nonzero software bytes on the retained 2026-07-16 probe. DRI3 is advertised only when a render-device provider exists. The paired classic/confined session launcher and strict verifier are implemented. Both isolated QEMU profiles pass exact text, presented centered cursor, button-gated Return, normal exit, and clean two-output retirement; promotion to `session` still awaits fresh paired target-hardware resize evidence. |
