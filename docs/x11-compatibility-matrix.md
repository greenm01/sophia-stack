...
| Kitty direct GLX bootstrap | `cargo run --offline -q -p sophia-cli -- x-authority-kitty-smoke` | GLVND selects Mesa; Kitty receives ARGB+sRGB FBConfig, direct context/window lifecycle, then uses DRI3/Present | `wire` | `candidate`: endian wire tests and lifecycle state pass; this build environment has no `/dev/dri`, so a render-node smoke with `first_error=none` and the guarded physical TTY3 session are still required before promotion. Indirect GLX is deliberately unsupported. |
