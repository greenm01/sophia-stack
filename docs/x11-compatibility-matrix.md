...
| Paired Milestone 3 session | `tools/live_session_milestone3_hardware_proof.sh` | run guarded two-xterm KMS evidence under classic shared-X and a fresh zero-capability confined namespace, including authenticated RandR delivery, configure-plus-pixels resize, exact physical keyboard input, and pointer-driven pixels | `hardware` | `implemented`; schema-13 completion and strict paired verification are ready, but fresh retained runs on the X13 are still required before promotion to `session`. |
