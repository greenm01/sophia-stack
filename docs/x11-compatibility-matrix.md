...
| xmobar status bar | `cargo run --offline -q -p sophia-cli -- x-authority-xmobar-smoke` plus `tools/start_sophia_xmonad_tty3.sh` with `SOPHIA_ENABLE_XMOBAR=true` | unmodified xmobar creates an override-redirect top bar that remains client-positioned, Engine-composed, and outside the blind WM layout stream | `session` | `automated proof passed; corrected physical proof pending`: unmodified xmobar 0.51.1 completed 215 requests across 28 opcodes with six committed CPU-buffer transactions, 25,929 nonzero bytes, and `first_error=none`; the gate now requires three sustained redraws and nonzero pixels in the newest transaction. The first physical run proved startup and redraw but exposed CPU/GPU occlusion: Sophia flattened CPU surfaces below Kitty's DMA-BUF. Mixed presentation now preserves CPU surface identity and interleaves both buffer classes by Engine order. The frontend remains application-agnostic, and neither xmobar nor xmonad is modified. `_NET_WM_STRUT_PARTIAL` work-area reservation remains an explicit follow-up. |
