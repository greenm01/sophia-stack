...
| GTK software dialog | Engine smoke plus `tools/live_session_milestone5_gtk_hardware_proof.sh` | GTK negotiates XKB/XGE/XI2, commits nonzero `MIT-SHM` pixels, accepts exact physical text and an OK-button click, redraws after resize, and exits normally | `engine` | Engine coverage is `proven` with `first_error=none`. The paired classic/confined session launcher and strict verifier are implemented; promotion to `session` awaits retained target-hardware evidence. |
